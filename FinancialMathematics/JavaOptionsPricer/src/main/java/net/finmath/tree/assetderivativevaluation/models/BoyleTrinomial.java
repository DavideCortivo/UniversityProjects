...
public class BoyleTrinomial extends AbstractRecombiningTreeModel {
	...
	public BoyleTrinomial(double spotPrice, double riskFreeRate, double volatility, double lastTime, int numberOfTimes, MultiplicativeDividendModel dividendModel) {
		...
		if(dividendModel instanceof ContinuousDividendYield) {
			...
			growthFactor  = Math.exp(getRiskFreeRate() * dt - contDiv.getDividendYield()*dt);
			M2 = Math.exp(2.0 * getRiskFreeRate() * dt - 2*contDiv.getDividendYield()*dt+ Math.pow(getVolatility(), 2) * dt);
		} else {
			growthFactor  = Math.exp(getRiskFreeRate() * dt);
			M2 = Math.exp(2.0 * getRiskFreeRate() * dt + Math.pow(getVolatility(), 2) * dt);
		}
	...
	protected RandomVariable buildSpotLevel(int k) {
		...
		for(int j=-k; j<=k; j++) {
			...
			level[idx] = S0 * Math.pow(u, upCount) * Math.pow(d, downCount)*dividendFactor; /**we added the dividend factor here*/
		}
	...
	protected RandomVariable conditionalExpectation(RandomVariable vNext, int k) {
		...
		for(int j=-k; j<=k; j++) {
			...
			now[idxNow] = (pu * up + pm * mid + pd * down)* getOneStepDiscountFactor(k); /** here we have added the one step discount factor */
		}
