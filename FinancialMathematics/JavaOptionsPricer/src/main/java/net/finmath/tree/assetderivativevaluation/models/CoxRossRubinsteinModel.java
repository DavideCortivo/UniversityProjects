...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	public CoxRossRubinsteinModel(double initialPrice, double riskFreeRate, double volatility, double lastTime, int numberOfTimes, MultiplicativeDividendModel dividendModel) {
		super(initialPrice, riskFreeRate, volatility, lastTime, numberOfTimes, dividendModel);
		...
		if(dividendModel instanceof ContinuousDividendYield) {
			...
			growth = Math.exp((r-contDiv.getDividendYield()) * dt); 
		} else {
	...
	* Binomial: at k level there are k+1 states
	...
	/** Build S_k[i] with agreement i = #down => S0 * u^(k-i) * d^i * dividendFactor .
	 * @param  k level of depth 
	 * @param  dividendFactor   value in percentage of the stock w.r.t. S0  after dividend event k
	 * */
	@Override
	protected RandomVariable buildSpotLevel(int k) {
		...
		for (int i = 0; i <= k; i++) {
			up = k - i;
			down = i;
			s[i] = s0 * Math.pow(u, up) * Math.pow(d, down)*dividendFactor; /* we added dividendFactor here */
		}
